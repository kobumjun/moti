/**
 * heroRoaming - safe viewport zones for walk-based roaming
 */
...
export function getZone(zone: ZoneKey, vw: number, vh: number): { x: number; y: number } {
  return {
    x: (ZONES_1280[zone]!.x / 1280) * vw,
    y: (ZONES_1280[zone]!.y / 800) * vh,
  };
}

export function clampToSafe(x: number, y: number, vw: number, vh: number): { x: number; y: number } {
  const minX = SAFE_PADDING + HERO_WIDTH / 2;
  const maxX = vw - SAFE_PADDING - HERO_WIDTH / 2;
  const minY = SAFE_PADDING + HERO_HEIGHT;
