...
import { getZone, clampToSafe, WALK_SPEED } from "./heroRoaming";
import { getPhrase } from "./heroDialogue";
import type { DialogueKey } from "./heroDialogue";
import type { ZoneKey } from "./heroRoaming";
import { getEventReaction } from "./heroActions";
import { pickVariation } from "./heroMotion";
import type { MotionVariation } from "./heroState";
import type { HeroMood } from "./heroState";
...
export default function HeroEngine() {
  ...
  const [state, setState] = useState<HeroEngineState>({
    x: 140,
    y: 600,
    facing: "right",
    speech: null,
  ...
  const walkTo = useCallback(
    (zone: ZoneKey, cb?: () => void) => {
      ...
      if (dist < ARRIVAL_THRESHOLD) {
        ...
        setState((s) => ({ ...s, x: t.x, y: t.y, action: "idleBreathing", walkFrame: 0, variation: pickVariation("idleBreathing") }));
        cb?.();
        return;
      }
      const dirX = dx / dist;
  ...
  const tick = useCallback(() => {
    ...
    setState((s) => {
      ...
      if (dist < ARRIVAL_THRESHOLD) {
        ...
        setTimeout(() => cb?.(), 0);
        return { ...s, x: targetX.current, y: targetY.current, action: "idleBreathing", walkFrame: 0, mood: "idle", variation: pickVariation("idleBreathing") };
      }
      const easeOut = Math.min(1, dist / 50);
      let newX = s.x + vx.current * dt * easeOut;
      let newY = s.y + vy.current * dt * easeOut;
      if (Math.hypot(newX - targetX.current, newY - targetY.current) < WALK_SPEED * dt * easeOut) {
        vx.current = 0;
        vy.current = 0;
        const cb = onArrive.current;
        onArrive.current = null;
  ...
  const runFlow = useCallback(() => {
    const flows: { zone: ZoneKey; action: string; speech?: DialogueKey; duration: number }[][] = [
      ...
      [
        ...
        { zone: "bottomCenter", action: "suspiciousLook", duration: 1800 },
        { zone: "pageList", action: "walk", duration: 0 },
      ...
      [
        { zone: "bottomCenter", action: "inspect", speech: "idle", duration: 2000 },
        { zone: "pageList", action: "walk", duration: 0 },
        { zone: "pageList", action: "capeAdjust", duration: 2200 },
        { zone: "center", action: "walk", duration: 0 },
        { zone: "center", action: "slightCrouch", duration: 1200 },
        { zone: "center", action: "jump", duration: 800 },
    ...
    if (!step) {
      flowStep.current = 0;
      flowIndex.current++;
      timers.current.push(setTimeout(runFlow, 2500));
      return;
    }
    ...
    const doPose = () => {
      setState((s) => ({
        ...
        mood: step.action === "idleBreathing" ? "idle" : "focused",
        variation: pickVariation(step.action as import("./heroState").BaseAction),
      }));
      if (step.speech) timers.current.push(setTimeout(() => setState((s) => ({ ...s, speech: null })), 2200));
      flowStep.current++;
    ...
    if (step.action === "walk" && dist > ARRIVAL_THRESHOLD) {
      ...
      if (step.speech) {
        timers.current.push(setTimeout(() => setState((s) => ({ ...s, speech: step.speech ? getPhrase(lang, step.speech) : null })), 400));
        timers.current.push(setTimeout(() => setState((s) => ({ ...s, speech: null })), 2600));
      }
      flowStep.current++;
      return;
    }
  ...
  const scheduleIdleDialogue = useCallback(() => {
    ...
    timers.current.push(
      setTimeout(() => {
        ...
        timers.current.push(setTimeout(() => setState((s) => s.speech ? { ...s, speech: null, action: "idleBreathing", variation: pickVariation("idleBreathing") } : s), 2400));
  ...
  useEffect(() => {
    ...
    posRef.current = { x: t.x, y: t.y };
    setState((s) => ({ ...s, visible: true, x: t.x, y: t.y, speech: getPhrase(lang, "intro"), variation: pickVariation("idleBreathing") }));
    vx.current = 0;
    vy.current = 0;
    timers.current.push(setTimeout(() => setState((s) => ({ ...s, speech: null })), 2400));
    timers.current.push(setTimeout(() => { flowStep.current = 1; runFlow(); }, 2600));
    scheduleIdleDialogue();
    const unsub = subscribeToCharacterEvents((d: CharacterEventDetail) => {
      ...
      if (d.type === "page_create") {
        walkTo("pageList", () => {
          ...
          timers.current.push(setTimeout(() => { setState((s) => ({ ...s, speech: null, action: "idleBreathing", mood: "idle", variation: pickVariation("idleBreathing") })); runFlow(); }, 2200));
        });
        return;
      }
      if (d.type === "save_click") {
        ...
        timers.current.push(setTimeout(() => setState((s) => ({ ...s, speech: null, action: "idleBreathing", mood: "idle", variation: pickVariation("idleBreathing") })), 2000));
        return;
      }
      if (d.type === "button_hover") {
        walkTo("saveArea", () => {
          ...
          timers.current.push(setTimeout(() => { setState((s) => ({ ...s, speech: null, action: "idleBreathing", mood: "idle", variation: pickVariation("idleBreathing") })); runFlow(); }, 1800));
        });
        return;
      }
      if (d.type === "language_changed") {
        ...
        timers.current.push(setTimeout(() => setState((s) => ({ ...s, speech: null, action: "idleBreathing", variation: pickVariation("idleBreathing") })), 1800));
        return;
      }
      if (d.type === "idle") {
        walkTo("logoutArea", () => {
          ...
          timers.current.push(setTimeout(() => { setState((s) => ({ ...s, speech: null, action: "idleBreathing", mood: "idle", variation: pickVariation("idleBreathing") })); runFlow(); }, 2800));
        });
        return;
      }
      if (d.type === "page_selected") {
        ...
        timers.current.push(setTimeout(() => setState((s) => ({ ...s, speech: null, action: "idleBreathing", variation: pickVariation("idleBreathing") })), 1500));
        return;
      }
      if (d.type === "text_change" || d.type === "typing_started" || d.type === "typing_stopped") {
        ...
        timers.current.push(setTimeout(() => setState((s) => ({ ...s, action: "idleBreathing", mood: "idle", variation: pickVariation("idleBreathing") })), reaction.duration));
      }
    });
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-40 overflow-visible" aria-hidden>
      ...
      >
        <AnimatePresence>
          {state.speech && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1" style={{ minWidth: 140, maxWidth: 220 }}>
              <div className={`relative px-4 py-2.5 rounded-xl bg-white border-2 border-slate-800 shadow-[4px_4px_0_0_rgba(15,23,42,0.4)] ${state.facing === "right" ? "rounded-bl-md" : "rounded-br-md"}`}>
                <p className="text-sm font-bold text-slate-900 leading-snug">{state.speech}</p>
                <div className="absolute left-1/2 -translate-x-1/2 top-full -mt-px w-0 h-0" style={{ borderLeft: "8px solid transparent", borderRight: "8px solid transparent", borderTop: "10px solid white", filter: "drop-shadow(0 2px 0 rgb(30 41 59))" }} />
              </div>
            </div>
          )}
        </AnimatePresence>
        <div className="relative w-full h-full overflow-visible">
          <HeroRenderer facing={state.facing} action={state.action} walkFrame={state.walkFrame} variation={state.variation} mood={state.mood} />
        </div>
      </div>
    </div>
  );
